  ...
  return (
    <main style={styles.main}>
      <section style={styles.card}>
        <h1 style={{ margin: 0 }}>Dashboard error</h1>
        <p style={{ opacity: 0.9, whiteSpace: 'pre-wrap' }}>{error?.message || 'A client error occurred.'}</p>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => reset()} style={styles.btn}>Try again</button>
          <a href="/login" style={{ ...styles.btn, textDecoration: 'none' }}>Go to Login</a>
        </div>
      </section>
    </main>
  );
